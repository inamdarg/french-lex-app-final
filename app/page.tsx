...
export default function Page() {
  ...
  useEffect(() => {
    ...
    recognition.onresult = (event: SpeechRecognitionEvent) => {
      ...
      alert(
        correct
          ? `Correct pronunciation: ${transcript}`
  ...
  const handleQuizSubmit = () => {
    const correct =
      userAnswer.trim().toLowerCase() === words[currentIndex].word.toLowerCase();
      ...
      {!quizMode ? (
        ...
        <Card className="mb-6 shadow-xl">
          <CardContent className="space-y-4">
            ...
            {summaryPublished && (
              <p className="mt-4 text-green-600 font-semibold text-center">✅ Summary shared to Instagram!</p>
            )}
          </CardContent>
        </Card>
      )}
